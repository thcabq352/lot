...
`01-foo.mp4` binds to shot `01` and **does not** rename the shot to `"01"`. Circle without `--take` exits non-zero (no GUI). Same circled takes twice is a no-op (no second `dailies.export` event). Writes `export.fcpxml` (FCPXML 1.9: format, `file://` URLs, cumulative spine) and `export.edl` (CMX 3600).
